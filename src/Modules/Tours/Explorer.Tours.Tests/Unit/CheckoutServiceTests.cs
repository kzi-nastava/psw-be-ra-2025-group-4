...
namespace Explorer.Tours.Tests.Unit
{
    public class CheckoutServiceTests
    {
        ...
        private class TokenRepoStub : ITourPurchaseTokenRepository
        {
            ...
            public TourPurchaseToken Create(TourPurchaseToken token) { Store.Add(token); return token; }
            public List<TourPurchaseToken> GetByTourist(int touristId) => Store.Where(t => t.TouristId == touristId).ToList();
            public bool Exists(int touristId, int tourId) => Store.Any(t => t.TouristId == touristId && t.TourId == tourId);
        }
        ...
        public void Checkout_creates_tokens_and_clears_cart()
        {
            ...
            cartRepo.Cart.TotalPrice.ShouldBe(0);
        }
